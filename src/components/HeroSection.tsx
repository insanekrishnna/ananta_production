...
const FloatingIcon = ({ icon }: { icon: (typeof FLOATING_ICONS)[number] }) => {
  ...
  return (
    <div
      className="absolute hidden md:flex w-12 h-12 rounded-[12px] bg-white/80 shadow-sm border border-[#e8e8e8] items-center justify-center animate-float"
      style={posStyle}
              ...
              icon.name === "Pinterest" ?
                ...
                icon.name === "Behance" ?
                  ...
                  icon.name === "Figma" ?
                    <FigmaIcon /> :
                    LucideIcon ?
                      <LucideIcon size={18} strokeWidth={1.2} className="text-[#1a1a1a]" /> :
                      <span className="text-sm font-medium text-[#1a1a1a]">{icon.letter}</span>
      }
    </div>
  );
};
  ...
  return (
    <section id="hero" className="relative min-h-[560px] pt-20 pb-0 overflow-hidden">
      ...
      <div ref={containerRef} className="relative z-10 flex flex-col items-center text-center px-[16px]">
        <h1 className="text-[36px] md:text-[56px] font-extrabold leading-[1.1] tracking-[-0.02em] text-[#1a1a1a] max-w-[960px] animate-hero-fade">
              ...
              part.accent && part.text === "companies" ?
                <span key={i} className={`relative inline-block ${openDropdown === "companies" ? "z-[999]" : ""}`}>
                  <button
                    ...
                    className="text-[#1a1a1a] underline decoration-[#1a1a1a]/20 underline-offset-4 hover:decoration-[#1a1a1a]/60 transition-all duration-200 cursor-pointer focus:outline-none"
                  >
                    {part.text}
                    <span
                      className="ml-1 text-[0.6em] opacity-40 inline-block transition-transform duration-200"
                  ...
                  >
                    <div className="bg-white border border-[#e8e8e8] rounded-[14px] shadow-[0_8px_30px_rgba(0,0,0,0.1)] p-2 min-w-[200px]">
                      {companyItems.map((item) => {
                        const Icon = item.icon;
                        return (
                          <button key={item.label} className="w-full flex items-center gap-3 px-4 py-3 rounded-[10px] text-left text-[15px] font-medium text-[#1a1a1a] hover:bg-[#f5f5f5] transition-colors duration-150" onClick={() => setOpenDropdown(null)}>
                ...
                part.accent && part.text === "innovation" ?
                  ...
                  part.accent ?
                    <span key={i} className="text-[#1a1a1a]">{part.text}</span> :
                    <span key={i}>{part.text}</span>
          )}
        </h1>

        <p
          className="text-[15px] text-[#888] leading-[1.65] max-w-[440px] mt-6 animate-hero-fade font-medium"
        ...
        <button
          className="mt-10 flex items-center gap-2 bg-[#1a1a1a] text-white rounded-pill h-14 text-[15px] font-semibold hover:bg-black hover:scale-[1.02] transition-all animate-hero-fade my-0 px-[19px]"
        ...
        <div
          className="w-full max-w-[720px] mt-12 rounded-[16px] border border-[#e8e8e8] shadow-[0_24px_64px_rgba(0,0,0,0.08)] overflow-hidden animate-hero-fade"
          style={{ animationDelay: "300ms" }}
        >
          <div className="h-9 bg-[#f5f5f5] flex items-center gap-1.5 px-4 border-b border-[#e8e8e8]">
            <div className="w-2.5 h-2.5 rounded-full bg-[#ccc]" />
            <div className="w-2.5 h-2.5 rounded-full bg-[#ddd]" />
            <div className="w-2.5 h-2.5 rounded-full bg-[#ddd]" />
          </div>
          <div className="bg-white p-4 flex gap-3 min-h-[200px]">
            <div className="hidden md:flex flex-col items-start gap-2.5 w-36 pr-3 border-r border-[#eee]">
              <img src={brandLogo} alt="Logo" className="w-5 h-5 object-contain mb-1" />
              <div className="h-2.5 w-28 bg-[#1a1a1a]/10 rounded-pill" />
              <div className="h-2.5 w-24 bg-[#f0f0f0] rounded-pill" />
              <div className="h-2.5 w-16 bg-[#f0f0f0] rounded-pill" />
              <div className="h-2.5 w-30 bg-[#f0f0f0] rounded-pill" />
            </div>
